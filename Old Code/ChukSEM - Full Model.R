...
### Load Model Code
source("./ChukSEM - Full Model Only.R")
...
constants <- list(
  ...
  ### Predictors
  era.gas = c(rep(1,19),rep(2, 27)), #Groupings for change in gas prices 
  era.awssi = c(rep(1,19),rep(2, 7), rep(1, 17)), #Groupings for change in gas prices 
...
### Predictors
une.init <- ifelse(is.na(une), mean(une), NA)
res.init <- ifelse(is.na(res), mean(res), NA)

wpdsi.init <- wpdsi
for(i in 1:2){wpdsi.init[,i] <- ifelse(is.na(wpdsi[,i]), mean(wpdsi[,i]), NA)}
pdsi.init <- pdsi
for(i in 1:2){pdsi.init[,i] <- ifelse(is.na(pdsi[,i]), mean(pdsi[,i]), NA)}
awssi.init <- awssi
for(i in 1:2){awssi.init[i,] <- ifelse(is.na(awssi[i,]), mean(awssi[i,]), NA)}
rabbits.init <- rabbits
for(i in 1:2){rabbits.init[,i] <- ifelse(is.na(rabbits[,i]), mean(rabbits[,i]), NA)}
raven.init <- as.vector(ifelse(is.na(bbs.df$raven), mean(bbs.df$raven), NA))
nharrier.init <- as.vector(ifelse(is.na(bbs.df$nharrier), mean(bbs.df$nharrier), NA))
